...
from cortstim.edp.objects.dataset.elecs import Contacts
from cortstim.edp.objects.dataset.basedataobject import BaseDataset
from cortstim.edp.utils.scalprecording import create_mne_topostruct_from_numpy
from cortstim.edp.utils.utils import compute_samplepoints
...
class TimeSeries(BaseDataset):
    ...
    Examples
    --------
    >>> from cortstim.edp.objects.dataset.eegts_object import TimeSeries
...
class EEGTs(TimeSeries):
    ...
    Examples
    --------
    >>> from cortstim.edp.objects.dataset.eegts_object import EEGTs
    >>> rawdata = np.random.rand((80,100))
    >>> metadata = dict()
...
class SEEGTs(TimeSeries):
    ...
    Examples
    --------
    >>> from cortstim.edp.objects.dataset.eegts_object import SEEGTs
    >>> rawdata = np.random.rand((80,100))
    >>> metadata = dict()
...
class ECOGTs(TimeSeries):
    ...
    Examples
    --------
    >>> from cortstim.edp.objects.dataset.eegts_object import ECOGTs
    >>> rawdata = np.random.rand((80,100))
    >>> metadata = dict()
